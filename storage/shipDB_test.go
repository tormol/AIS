...
func TestGeoJSON(t *testing.T) {
	...
	for _, c := range cases {
		i := ship{c.mmsi, Mmsi(c.mmsi).Owner(), Mmsi(c.mmsi).CountryCode(), ShipInfo{Length: c.length, Dest: c.dest, Callsign: c.call, ShipName: c.name}, ShipPos{BowHeading: c.heading}, []geo.Point{}, &sync.Mutex{}}
